#!/bin/bash
# v12_run.sh
# Runs the v12 experiment sequence for each of 8 models sequentially,
...
#
# v12 changes from v11: DeepSeek-R1:70b dropped (distillation confound).
#
# Usage:
#   ./v12_run.sh v12_cold_label-vegan/v12_qs_cold_label-vegan.md
#   ./v12_run.sh v12_cold_label-ar/v12_qs_cold_label-ar.md http://192.168.1.100:11434

set -e

if [ -z "$1" ]; then
    echo "Usage: $0 <qs_file> [host]"
    echo "Example: $0 v11_qs_present-warm.md"
    exit 1
fi
...
for MODEL in "${MODELS[@]}"; do
    ...
    if python3 "${SCRIPT_DIR}/v12_run.py" \
        --qs "${QS_FILE}" \
        --model "${MODEL}" \
        --host "${HOST}" \
