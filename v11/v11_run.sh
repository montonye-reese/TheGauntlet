#!/bin/bash
# v11_run.sh
# Runs the v11 experiment sequence for each of 9 models sequentially,
...
#
# Usage:
#   ./v11_run.sh v11_qs_present-warm.md                          # default localhost
#   ./v11_run.sh v11_qs_present-warm.md http://192.168.1.100:11434  # custom host
...
echo "========================================================"
echo "  8 DEGREES — v11 ALL MODELS RUNNER"
echo "  $(date)"
...
for MODEL in "${MODELS[@]}"; do
    ...
    if python3 "${SCRIPT_DIR}/v11_run.py" \
        --qs "${QS_FILE}" \
        --model "${MODEL}" \
        --host "${HOST}" \
