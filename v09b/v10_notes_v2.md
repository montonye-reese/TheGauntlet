...
### Questions
- **CK0 phrasing** — "where are we with the framework as it stands right now" is redundant ("as it stands" + "right now"). Clean up in v10. Candidate: "Pause here for a sec — where are we with the framework?"
- **PA / P15c split** — consider moving the alignment goals question out of PA
  and into a new P15c. PA would then focus purely on the "set against the
  original / what does the distance say" reflection. The alignment goals
  question becomes its own closing prompt, giving it room to breathe rather
  than being appended to PA.
...
- **seated-warm** — drafted 2026-04-08. P1 establishes division of labor:
  model owns the framework, interviewer asks questions to develop and refine.
  "Where are we" at checkpoints. "Tell me" and "I'm curious" as seating moves.
  "Let's see what we've got" at P14. MUST NOT include stakes-loading
